...
export function HrEmployees({
  ...
  const submit = async (event: FormEvent) => {
    ...
    await apiRequest(editingId ? `/api/hr/employees/${editingId}` : '/api/hr/employees', {
      method: editingId ? 'PATCH' : 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    })
    setShowForm(false)
    onMessage(editingId ? 'Đã cập nhật hồ sơ nhân viên.' : 'Đã tạo hồ sơ nhân viên. Phần lương/phụ cấp chờ Kế toán trưởng thiết lập.')
    await load()
  }
  ...
  return (
    <section style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={{ ...cardStyle, padding: 14, background: '#f8fafc', fontSize: 13, color: '#475569' }}>
        ...
        Quyền được xác định tự động: chức vụ Admin trong nhánh IT &amp; ADMIN nhận HR_ADMIN; nhân viên IT dùng tài khoản quản trị chung admin_sealink; các tài khoản cá nhân khác nhận USER.
      </div>
      {showForm && (
        <form onSubmit={submit} style={{ ...cardStyle, display: 'grid', gap: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <h3 style={{ margin: 0 }}>{editingId ? 'Chỉnh sửa hồ sơ' : 'Thêm nhân viên mới'}</h3>
            <button type="button" style={buttonStyle} onClick={() => setShowForm(false)}>Đóng</button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 10 }}>
            {[
              ['machine_employee_id', 'Mã máy chấm công'],
              ['biometric_id', 'Mã máy phụ'],
              ['full_name', 'Họ và tên'],
              ['notion_name', 'Tên Notion'],
              ['employee_code', 'Mã nhân viên'],
              ['position', 'Chức vụ'],
              ['company_email', 'Email công ty'],
              ['phone_number', 'SĐT cá nhân'],
              ['company_phone_number', 'SĐT công ty'],
              ...
              <label key={key}>{label}
                <input
                  ...
                  value={form[key] || ''}
                  required={key === 'machine_employee_id' || key === 'full_name'}
                  onChange={(e) => setForm((previous: any) => ({ ...previous, [key]: e.target.value }))}
                />
              </label>
            ))}
            <label>Phòng ban
          ...
          <p style={{ margin: 0, color: '#b45309', fontSize: 13 }}>
            Admin vận hành không thể nhập hoặc thay đổi lương, phụ cấp và bonus. Các mục đó do Kế toán trưởng thiết lập.
          </p>
          <button style={buttonStyle} type="submit">Lưu hồ sơ</button>
        </form>
      )}
...
export function ItOperations({ apiRequest, view }: { apiRequest: ApiRequest; view: 'backups' | 'audit' }) {
  ...
  const run = async () => {
    setRunning(true)
    try {
      await apiRequest('/api/it/backups/run', { method: 'POST' })
      await load()
    } catch (reason: any) {
      setError(String(reason?.message || reason))
    } finally {
  ...
  if (error) return <div className="status-message error">{error}</div>
  if (!data) return <div className="status-message">Đang tải dữ liệu IT…</div>
  if (view === 'backups') {
    return (
      <section style={{ display: 'grid', gap: 16 }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ maxHeight: 420, overflow: 'auto' }}>
            <table style={{ width: '100%' }}><thead><tr><th>Tệp</th><th>Thời điểm</th><th>Kích thước</th><th>SHA-256</th></tr></thead>
              <tbody>{data.backups.map((item: any) => <tr key={item.name}><td>{item.name}</td><td>{new Date(item.created_at).toLocaleString('vi-VN')}</td><td>{Math.round(item.size_bytes / 1024)} KB</td><td><code>{item.sha256?.slice(0, 16)}…</code></td></tr>)}</tbody>
            </table>
          </div>
        </div>
      </section>
    ...
    <section style={{ display: 'grid', gap: 16 }}>
      <div style={{ ...cardStyle, display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'end', justifyContent: 'space-between' }}>
        ...
        <div className="app-action-toolbar">
          <input type="month" style={inputStyle} value={period} onChange={(e) => onPeriodChange(e.target.value)} />
                    ...
                    >
                      ...
                      {!weekend && (
                        <div style={{ display: 'grid', gap: 6, marginTop: 12, fontSize: 11 }}>
                          {row.check_in ? <div><span style={{ color: '#10b981' }}>●</span> <span style={{ color: '#64748b' }}>In:</span> <b style={{ marginLeft: 4 }}>{String(row.check_in).slice(0, 5)}</b></div> : <div style={{ color: '#94a3b8' }}>● Không quẹt</div>}
                          ...
                          {rawScans.length > 0 && (
                            <div className="group" style={{ position: 'relative' }}>
                              ...
                              <div className="hidden group-hover:block group-focus-within:block" style={{ position: 'absolute', zIndex: 20, bottom: 38, left: '50%', width: 210, transform: 'translateX(-50%)', border: '1px solid #bfdbfe', borderRadius: 12, background: '#e8f0ff', padding: 10, boxShadow: '0 14px 34px rgba(15, 23, 42, 0.22)' }}>
                                <p style={{ margin: '0 0 8px', color: '#64748b', fontSize: 9, fontWeight: 800, textTransform: 'uppercase' }}>Lịch sử quẹt thẻ trong ngày</p>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 5 }}>
                                  {rawScans.map((scan: unknown, index: number) => <span key={`${row.work_date}-${index}`} style={{ borderRadius: 5, background: '#ffffff', padding: '4px 5px', textAlign: 'center', color: '#334155', fontSize: 10 }}>{String(scan).slice(0, 5)}</span>)}
                                </div>
                                {row.override_reason && <p style={{ margin: '8px 0 0', color: '#b45309', fontSize: 10 }}>Đã điều chỉnh: {row.override_reason}</p>}
                              </div>
                            </div>
                          )}
                        </div>
